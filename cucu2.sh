...
if [ -z "$proxy" ]; then
curl -L -X POST https://api.telegram.org/bot$token/sendMessage -F chat_id="$chat_id" -F text=$text > $ftb"out0.txt"
else
curl --proxy $proxy -L -X POST https://api.telegram.org/bot$token/sendMessage -F chat_id="$chat_id" -F text=$text > $ftb"out0.txt"
fi
